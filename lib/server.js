...
function createServer({ port, apiToken, rateLimit, db, onUpdate, onReset }) {
  ...
  function handleStatus(req, res) {
    let usage = null;
    try { usage = db.getLatest(); } catch {}
    send(res, 200, { ok: true, usage, usingFallback: db.usingFallback ?? false }, req._rlHeaders);
  }
  ...
  async function handleUpdate(req, res) {
    ...
    try {
      usage = clean.add
        ? db.upsert(sessionId, sessionStart, clean.model, clean.context_window, clean)
        : db.replace({ ...clean, session_id: sessionId });
    } catch (e) {
  ...
  function handleReset(req, res) {
    try { db.reset(); } catch (e) { console.error('[server] Reset error:', e.message); }
    onReset();
    send(res, 200, { ok: true }, req._rlHeaders);
  }
